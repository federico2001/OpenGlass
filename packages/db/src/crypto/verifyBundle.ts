...
/**
 * SPEC §7.6 / §12. Runs the full independent-verification algorithm: anyone can call this
 * (the SDKs, the web UI, `POST /v1/verify`, and the worker before it issues a record) given
 * a bundle plus a list of trusted platform keys from OUTSIDE the bundle (pinned in the
 ...
 * before `kind` existed — and `"attestation"`, Prompt 20). Only step 2 (genesis) actually
 * differs between the two; steps 1, 3, and 4 are kind-agnostic by construction (see the
 * field-reuse notes on `MessageEnvelope`/`CloseStatement`/`RecordStatement` in protocol.ts)
 * — this is what "reuse the session bundle format so verify() works unchanged" means in
 * practice: one function, one bundle shape, a single small branch.
 *
